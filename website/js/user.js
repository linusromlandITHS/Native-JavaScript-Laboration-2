...
displayName = (user) => {
    ...
    document
        ...
        .setAttribute("content", `Github Statistics for user ${user.login}`);
};
...
displayCommitAmount = (data) => {
    ...
    if (commits) {
        ...
        commitAmount.textContent = `Commits (${data.day[0]} - ${
            data.day[data.day.length - 1]
        }): ${commits}`;
...
displayLatestCommits = (arr) => {
    ...
    while (commits.length < show && arr.length > 0) {
        if (arr[i].type === "PushEvent") {
            // Checkes that event is of type PushEvent.
    ...
    for (let i = 0; i < amount; i++) {
        ...
        date.textContent = `${moment(
            element.commitInformation.created_at
        ).calendar(null, {
...
eventArrayToChart = (eventArray) => {
    ...
    for (let i = 0; i < eventArray.length; i++) {
        ...
        const diff = date.diff(
            moment(object.day[object.day.length - 1]),
            "days"
        );
        if (diff > 1) {
        ...
        const index = object.day.findIndex(
            (e) => e === date.format("YYYY-MM-DD")
        ); //Checks if date is in date array
        if (index === -1) {
            // if date is not in array and either adds it or updates amount.
        ...
        const repoIndex = topRepositories.findIndex(
            (e) => e.repo === commit.repo.name
        ); //Checks if repo is in repo array
        if (repoIndex === -1) {
