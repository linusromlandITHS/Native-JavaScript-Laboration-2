...
form.addEventListener("submit", async (e) => {
    ...
    if (input.value && populationInput.value) {
        ...
        if (request.status === 201) {
            ...
            responseMessage(
                `Added city "${input.value}" to Cities API`,
                "successMessage"
            );
            input.value = "";
...
renderCitites = async () => {
    ...
    if (JSON.stringify(cities) != JSON.stringify(_cities)) {
        ...
        cities.forEach((city) => {
            ...
            cityPopulation.textContent = `${Intl.NumberFormat().format(
                city.population
            )} ${city.population == "1" ? "inhabitant" : "inhabitants"}`;
...
editCityValue = async (name, pop, object) => {
    console.log(
        `Updated the city ${object.name}'s information. (ID=${object.id})`
    );
    const obj = {};
    if (name.value && object.name != name.value) obj.name = name.value;
    if (pop.value && object.population != parseInt(pop.value))
        obj.population = parseInt(pop.value);

    if (object.id && !(!name.value && !pop.value)) {
        const request = await fetch(
            `https://avancera.app/cities/${object.id}`,
            {
...
updateDeleteModal = (object) => {
    ...
    document.querySelector(
        "#delConfirmText"
    ).textContent = `You are about to delete the city ${object.name}! Are you sure you want to do this?`;
...
checkStatus = () => {
    submitButton.disabled = !(
        input.value.length > 0 && populationInput.value > 0
    );
};
